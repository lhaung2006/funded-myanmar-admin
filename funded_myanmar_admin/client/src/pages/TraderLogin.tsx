...
export default function TraderLogin() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      fetch("/api/trader/verify", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      })
        .then((res) => {
          if (res.ok) {
            setLocation("/trader/dashboard");
          } else {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/trader/login", {
        method: "POST",
      ...
      localStorage.setItem("traderAccountNumber", data.accountNumber);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">Trader Dashboard</p>
        </div>

        {/* Login Card */}
        <Card className="border-slate-700 bg-slate-900/50 backdrop-blur-sm">
          <CardHeader className="space-y-1">
            <CardTitle className="text-white">Trader Login</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button
                type="submit"
                disabled={loading || !accountNumber || !password}
            ...
            <div className="mt-6 pt-6 border-t border-slate-700 space-y-3">
              <p className="text-xs text-slate-500 text-center">
                Demo Trader: ACC-001 / smaung2006
              </p>
        ...
        <div className="text-center mt-8">
          <p className="text-sm text-slate-500">
            Admin account?{" "}
            <a href="/admin/login" className="text-green-400 hover:text-green-300 font-semibold">
              Login as Admin
            </a>
          </p>
        </div>
